...
def process(outFname, files):
    ...
    for f in range(0, len(files)):
        ...
        for node in xmlFile.getElementsByTagName("testcase"):
            ...
            for k,v in (list(node.attributes.items())):
                if k == "name" or k == "status" or k == "time" or k == "classname" or k == "timestamp" or k == "result":
                    continue
    ...
    sortedValues = list( v for k,v in sorted( finalCtr.items( ) ) )
    htmlDocument = htmlDocumentText.format(rows = generateElements( sortedValues, True), tableHeader = tableHeader, report = reportHeader + report)
